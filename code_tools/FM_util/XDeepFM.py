...
class XDeepFM(object):
    def __init__(self,
                 ...
                 dropout_deep=[0.5, 0.5, 0.5],  # DNN部分的 dropout层  要比deep_layer 长度多1
        ...
        assert (len(dropout_cin) == len(cin_param)),\
            "Parameter dropout_cin and parameter cin_param must be read equal in length"
        assert len(deep_layers)+1 == len(dropout_deep), \
            "Parameter dropout_cin length is larger than parameter cin_param length 1 "
        ...
        self.every_epoch_train = every_epoch_train
        self._init_graph()
    ...
    def _deep_net(self):   # 深度部分网络结构进行定义
        ...
        self.y_deep = tf.reshape(self.embeddings, shape=[-1, self.field_size*self.embedding_size])  # Batch *F*k
        self.y_deep = tf.compat.v1.nn.dropout(self.y_deep, self.dropout_keep_deep[0])  # 即第一部分的
        # print(self.y_deep)
        for i in range(layer_num):  # 遍历所有的层
            ...
            with tf.compat.v1.variable_scope("deep_layer_%d" % i, reuse=tf.compat.v1.AUTO_REUSE):
                if i == 0:
                    shape = [input_size, self.deep_layers[i]]
                else:
                ...
                self.y_deep = tf.compat.v1.nn.dropout(self.y_deep, self.dropout_keep_deep[i+1])   # 加上dropout 层
    ...
    def _cin_net(self, bias=False):  # cin 网络结构
        ...
        final_result = []
        self.split_tensor_h0 = tf.compat.v1.split(hidden_nn_layers[0], self.embedding_size*[1], 2)
        # 进行拆分， 拆分成k个 batch*h0*1 小张量
        # self.layer_params   为每层需要确定的行， Hk
        for idx, layer_size in enumerate(self.cin_param):
            # 人工指定特征数 layer_size
            # idx为层索引
            # print(hidden_nn_layers[-1])
            # hidden_nn_layers[-1]  batch*(H_(0)*D
            print(hidden_nn_layers)
            split_tensor_hk = tf.compat.v1.split(hidden_nn_layers[-1], self.embedding_size*[1], 2)
            ...
            # print(dot_result_hkm)
            # print(field_nums[0])
            # print(field_nums[-1])

            dot_result_hko = tf.reshape(dot_result_hkm, shape=[self.embedding_size, -1, field_nums[-1]*field_nums[0]])
            # 将上述尺寸变成D*batch*(H_(K)*m)

            dot_result = tf.compat.v1.transpose(dot_result_hko, perm=[1, 0, 2])
            ...
            cin_unit = tf.compat.v1.transpose(cin_second, perm=[0, 2, 1])  # 转置得到 batch*(H_(K)*D
            ...
            if self.direct:
                ...
                next_hidden = tf.compat.v1.transpose(cin_unit, perm=[0, 2, 1])
                ...
                if idx != len(self.cin_param)-1:
                    # 非最后一层
                    next_hidden, direct_connect = tf.compat.v1.split(cin_unit, 2*[int(layer_size/2)], 2)  # 取一半值，将D进行分解
                    # 一半用于下一层输入，一半丢给输出
                    final_len += int(layer_size/2)
                    next_hidden = tf.compat.v1.transpose(next_hidden, perm=[0, 2, 1])  # Batch_size *m *D
                else:
                    # 最后一层
    ...
    def _filter_cin(self, params, f_dim=2):
        ...
        if self.reduce_D:
            with tf.compat.v1.variable_scope("reduce_D", reuse=tf.compat.v1.AUTO_REUSE):
                ...
                filters = tf.compat.v1.transpose(filters_o, perm=[0, 2, 1])   # 变成1 *(m*hm)*N
    ...
    def _loss(self, logits):   # 损失函数
        ...
        if self.loss_type == "logloss":
            self.loss = tf.compat.v1.losses.log_loss(labels=self.label, predictions=self.pred)
    ...
    def _inference(self, prob=False):   # 定义前向推断
            ...
            with tf.compat.v1.variable_scope("linear_part", reuse=tf.compat.v1.AUTO_REUSE):
                ...
                # 做的是FM 中的<wx> 操作，无偏置
                self.embedding_out = tf.reduce_sum(tf.multiply(self.line_out, feat_value), 2)  # 页内列相加，None*F*1
                self.embedding_out = tf.compat.v1.nn.dropout(self.embedding_out, self.dropout_keep_cin[0])
            print(self.embedding_out)
    ...
    def evaluate(self, real_label, evaluate_type="acc"):   # 评估模块
        ...
        if self.evaluate_type == "all" or "acc":
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            tf.compat.v1.set_random_seed(self.random_seed)  # 设定随机种子
            self.feat_index = tf.compat.v1.placeholder(tf.int32, shape=[None, None],
                                                       name="feat_index")
            # None*F 样本数为None， F为field的个数，原始特征的数目
            # 为索引，查找随机矩阵中的嵌入向量
            self.feat_value = tf.compat.v1.placeholder(tf.float32, shape=[None, None],  # None*F
                                                       name="feat_value")
            self.label = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name="label")  # None*1
            self.dropout_keep_cin = tf.compat.v1.placeholder(tf.float32, shape=[None], name="dropout_keep_cin")  # None
            ...
            self.global_step = tf.Variable(0, trainable=False, name="step")  # 初始化变量为0 ，不可训练

            self.decay_step = int(self.num_example / self.batch_size)  # 衰减步系数， 必须为整形
            self._skill(self.global_step, self.decay_step)  # 使用 滑动平均与  指数衰减
            ...
            tf.compat.v1.add_to_collection("loss", self.loss)  # 便于离线断点操作

            updata_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)  # 网络层中所有的变量，训练之前完成

            # 如有需要打印
            # for i in updata_ops:
            #     print(i)

            train_step = self._optimizer(global_step=self.global_step)  # 当前步数， 优化器，即反向传播

            if self.movingaverage:
                train_ops = [train_step, self.variable_averages_op]
            else:
                train_ops = [train_step]

            with tf.compat.v1.control_dependencies(train_ops):
            ...
            self.saver = tf.compat.v1.train.Saver(max_to_keep=5)  # 默认保存5个

    def train(self, data, save_params, every_save=1000, every_eval=1000, early_stop=False,
        ...
        with tf.compat.v1.Session(graph=self.graph) as sess:
            ...
            for epoch in range(self.traning_step):
                ...
                # 获取批量数据
                start = time()
                if self.every_epoch_train:  # 每一轮都遍历一遍整个数据
                    for iters in range(int(self.num_example/self.batch_size)):
                        xi, xv, label = train_data.next_batch(self.batch_size, list2multlist=True)
                ...
                if epoch % every_save == 0:
                    if self.verbose:
                        print("After %d training step(s), loss on training batch is %g, spent time is %.4f"
                              % (step, loss_value, end-start))
                    else:
                        print("After %d traning step(s),loss on training batch is %g" % (step, loss_value))
                    self.saver.save(sess, os.path.join(save_params["model_save_path"], save_params["model_name"]),
                ...
                if train_test_split:    # 有验证集 则进行评估处理
                    ...
                    if epoch % every_eval == 0:  # 每隔一定轮数，进行操作
                        valid_num += 1
                        xi_v, xv_v, label_v = valid_data.next_batch(valid_data.num_examples)  # 获取整个验证集的数据
                        ...
                        if self.evaluate_type == "all":
                            ...
                            pred, label_v = self.___predict(sess=sess, predict_data=valid_data, ifsplit=True)
                            valid_auc = roc_auc_score(y_true=np.array(label_v), y_score=pred)
                            ...
                            valid_auc = roc_auc_score(y_true=label_v, y_score=pred)
                        ...
                        print("After %d validing step(s),loss on validata batch is %g"
    ...
    def ___predict(self, sess, predict_data, ifsplit=False):  # 不能供外部使用
        """
        :param predict_data:
        :param ifsplit:  切割多个进行预测 True 为切割， False为不切割,默认不使用，
        原因，当整个预测样本数 = batch_size的整数倍时，可使用分割，
        :return:
        """
        predict_num_example = predict_data.num_examples

        # 加条件强制限制
        if predict_num_example ==  int(predict_num_example/self.batch_size)*self.batch_size:
            ifsplit =True

        if ifsplit:
            batch_conpat_predict_list = list()
            batch_conpat_predictlabel_list =list()
            for eval_epoch in range(int(predict_num_example/self.batch_size)):
                predict_xi, predict_xv, predict_label = predict_data.next_batch(self.batch_size,list2multlist=False)
                batch_out = sess.run(self.pred, feed_dict= self.set_on_batch(predict_xi, predict_xv,
                                                                             predict_label, False))
                batch_conpat_predict_list.append(batch_out)
                batch_conpat_predictlabel_list.append(predict_label)
            predict_out = np.concatenate(batch_conpat_predict_list, axis=0)
            predict_label = np.concatenate(batch_conpat_predictlabel_list, axis=0)

        else:
            predict_xi, predict_xv, predict_label = predict_data.next_batch(predict_num_example)
            predict_out = sess.run(self.pred, feed_dict=self.set_on_batch(predict_xi, predict_xv,
                                                                          predict_label, False))
            print(predict_out)
            print(type(predict_out))
