...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # 作为默认的计算图
            ...
            # model
            self.embeddings = tf.nn.embedding_lookup(self.weights["feature_embeddings"], self.feat_index)  # [?,?,k]

            feat_value = tf.reshape(self.feat_value, shape=[-1, self.field_size, 1])   # [?,F, 1]

            self.embeddings = tf.multiply(self.embeddings, feat_value)  # shape=(?,F,K)
            ...
            # --------------------------------------DeepFM-----------------------------------------
            # print(self.y_first_order)    # shape =(batch_size, field_size)
            # print(self.y_second_order)    # shape =(batch_size, embedding_size)
            # print(self.y_deep)    # shape =(batch_size, deep_last_node)
