...
class BacthDataset(object):  # 制作自己的数据集
    ...
    def next_batch(self, batch_size, shuffle=True, list2multlist=True):
        """
        :param batch_size:  批量大小，int
        :param shuffle:  是否 开启混洗
        :param list2multlist:  # 单个列表转成一个多个列表的列表，True 表示开启， False 表示不开启
        :return:
        """
        ...
        if start + batch_size > self._num_examples:
            ...
            if list2multlist:
                return np.concatenate((xi_rest_part, xi_new_part), axis=0).tolist(), np.concatenate(
                    (xv_rest_part, xv_new_part), axis=0).tolist(), np.reshape(np.concatenate(
                    (label_rest_part, label_new_part), axis=0).tolist(), [-1, 1]).tolist()
            else:
                return np.concatenate((xi_rest_part, xi_new_part), axis=0).tolist(), np.concatenate(
            ...
            if list2multlist:
                return self._xi[start:end].tolist(), self._xv[start:end].tolist(), \
                       np.reshape(self._labels[start:end].tolist(), [-1, 1]).tolist()
            else:
                return self._xi[start:end].tolist(), self._xv[start:end].tolist(), \
                       np.reshape(self._labels[start:end].tolist(), [-1, 1]).tolist()
...
def scale_data(xi,xv,label,ratio=1.0):
    data_len = int(len(label)*ratio)
    xi = xi[0:data_len]
    xv = xv[0:data_len]
...
if __name__ == "__main__":
    ...
    print(dataset.next_batch(20, list2multlist=True))
    # print(df.head())
